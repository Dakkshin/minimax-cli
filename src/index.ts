...
import { MiniMaxAgent } from "./agent/minimax-agent.js";
...
async function saveCommandLineSettings(
  ...
  try {
    ...
    if (apiKey) {
      ...
      console.log("✅ API key saved to ~/.minimax/user-settings.json");
    }
    if (baseURL) {
      manager.updateUserSetting("baseURL", baseURL);
      console.log("✅ Base URL saved to ~/.minimax/user-settings.json");
    }
  } catch (error) {
    console.warn(
...
function loadModel(): string | undefined {
  ...
  let model = process.env.MINIMAX_MODEL;
...
async function handleCommitAndPushHeadless(
  ...
  try {
    const agent = new MiniMaxAgent(apiKey, baseURL, model, maxToolRounds);
...
program
  .name("minimax")
  .description(
    "A conversational AI CLI tool powered by MiniMax with text editor capabilities"
  )
  .version("1.0.1")
  .argument("[message...]", "Initial message to send to MiniMax")
  .option("-d, --directory <dir>", "set working directory", process.cwd())
  .option("-k, --api-key <key>", "MiniMax API key (or set MINIMAX_API_KEY env var)")
  .option(
    "-u, --base-url <url>",
    "MiniMax API base URL (or set MINIMAX_BASE_URL env var)"
  )
  .option(
    "-m, --model <model>",
    "AI model to use (e.g., minimax-01, minimax-pro) (or set MINIMAX_MODEL env var)"
  )
  .option(
    "-p, --prompt <prompt>",
  ...
  .action(async (message, options) => {
    ...
    try {
      ...
      if (!apiKey) {
        console.error(
          "❌ Error: API key required. Set MINIMAX_API_KEY environment variable, use --api-key flag, or set \"apiKey\" field in ~/.minimax/user-settings.json"
        );
        process.exit(1);
      }
      ...
      const agent = new MiniMaxAgent(apiKey, baseURL, model, maxToolRounds);
      console.log("🤖 Starting MiniMax CLI Conversational Assistant...\n");
      ...
      console.error("❌ Error initializing MiniMax CLI:", error.message);
      process.exit(1);
    }
  });
...
gitCommand
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (!apiKey) {
        console.error(
          "❌ Error: API key required. Set MINIMAX_API_KEY environment variable, use --api-key flag, or save to ~/.minimax/user-settings.json"
        );
        process.exit(1);
      }
