...
const BASE_MINIMAX_TOOLS: MiniMaxTool[] = [
  ...
  },
  {
    ...
    function: {
      name: "web_search",
      description: "Free web search using DuckDuckGo HTML scraping. Returns structured results with title, link, and snippet for each result.",
      parameters: {
        type: "object",
        properties: {
          query: {
          ...
          fetch_pages: {
            type: "boolean",
            description: "Fetch and extract content from top result pages for higher quality answers (default: false)",
          },
        },
        required: ["query"],
      },
...
const MORPH_EDIT_TOOL: MiniMaxTool = {
  type: "function",
  function: {
    name: "edit_file",
...
function buildMiniMaxTools(): MiniMaxTool[] {
  const tools = [...BASE_MINIMAX_TOOLS];
...
export const MINIMAX_TOOLS: MiniMaxTool[] = buildMiniMaxTools();
...
export function addMCPToolsToMiniMaxTools(baseTools: MiniMaxTool[]): MiniMaxTool[] {
  if (!mcpManager) {
    return baseTools;
  }
  
  const mcpTools = mcpManager.getTools();
  const miniMaxMCPTools = mcpTools.map(convertMCPToolToMiniMaxTool);
  
  return [...baseTools, ...miniMaxMCPTools];
}

export async function getAllMiniMaxTools(): Promise<MiniMaxTool[]> {
  const manager = getMCPManager();
  ...
  return addMCPToolsToMiniMaxTools(MINIMAX_TOOLS);
}
