...
/**
 * Scan directory for files recursively (limited depth for performance)
 */
function scanFiles(dir: string, maxDepth: number = 2, currentDepth: number = 0): string[] {
  const files: string[] = [];
...
/**
 * Filter files based on user input after @
 */
function filterFiles(files: string[], input: string): string[] {
  if (!input) return files.slice(0, 10); // Show first 10 files if no input
...
/**
 * Parse @filename.ext references from user input
 */
function parseFileReferences(input: string): { cleanInput: string; fileReferences: FileReference[] } {
  const fileReferenceRegex = /@([a-zA-Z0-9_.-]+(?:\/[a-zA-Z0-9_.-]+)*)/g;
  ...
  while ((match = fileReferenceRegex.exec(input)) !== null) {
    ...
    fileReferences.push({ filename, content: "" });
    // Remove the @ reference from the clean input
    cleanInput = cleanInput.replace(match[0], "").trim();
  }

  return { cleanInput, fileReferences };
}

/**
 * Read the contents of referenced files
 */
async function readReferencedFiles(fileReferences: FileReference[]): Promise<void> {
  for (const ref of fileReferences) {
    try {
      const resolvedPath = path.resolve(ref.filename);

      // Check if file exists
      if (!fs.existsSync(resolvedPath)) {
        ref.error = `File not found: ${ref.filename}`;
        continue;
      }
      ...
      if (stats.isDirectory()) {
        ...
        ref.content = `Directory: ${ref.filename}\nContents:\n${files.join('\n')}`;
      } else {
        // Read file content
        ref.content = fs.readFileSync(resolvedPath, 'utf-8');
      }
    } catch (error: any) {
      ref.error = `Error reading file: ${error.message}`;
    }
  }
}

/**
 * Format file references for inclusion in conversation context
 */
function formatFileContext(fileReferences: FileReference[]): string {
  ...
  for (const ref of fileReferences) {
    ...
    if (ref.error) {
      context += `Error: ${ref.error}\n`;
    } else {
      // Truncate very large files for context
      const maxLength = 10000;
      const content = ref.content.length > maxLength
        ? ref.content.substring(0, maxLength) + `\n\n... (file truncated, showing first ${maxLength} characters)`
        : ref.content;
...
export function useInputHandler({
  ...
  const handleSpecialKey = (key: Key): boolean => {
    ...
    if (showFileSuggestions) {
      ...
      const filteredFiles = filterFiles(availableFiles, afterAt);
      ...
      if (filteredFiles.length === 0) {
        ...
        if (key.tab || key.return) {
          ...
          const selectedFile = filteredFiles[safeIndex];
  ...
  const handleInputSubmit = async (userInput: string) => {
    ...
    if (userInput.trim()) {
      ...
      const { cleanInput, fileReferences } = parseFileReferences(userInput);

      // Read referenced files if any
      if (fileReferences.length > 0) {
        await readReferencedFiles(fileReferences);

        // Show feedback about referenced files
        if (fileReferences.some(ref => !ref.error)) {
          const successFiles = fileReferences.filter(ref => !ref.error);
          const errorFiles = fileReferences.filter(ref => ref.error);

          const feedbackEntry: ChatEntry = {
            type: "assistant",
            content: `📎 Referenced ${successFiles.length} file(s): ${successFiles.map(f => f.filename).join(', ')}${
              errorFiles.length > 0 ? `\n⚠️  ${errorFiles.length} file(s) had errors: ${errorFiles.map(f => `${f.filename} (${f.error})`).join(', ')}` : ''
            }`,
            timestamp: new Date(),
          };
          setChatHistory((prev) => [...prev, feedbackEntry]);
        }
      }

      // Use clean input (without @ references) plus file context
      const inputWithContext = cleanInput + formatFileContext(fileReferences);

      const directCommandResult = await handleDirectCommand(inputWithContext);
      if (!directCommandResult) {
        await processUserMessage(inputWithContext);
      }
    }
  };
  ...
  const handleInputChange = (newInput: string) => {
    ...
    if (newInput.startsWith("/")) {
      ...
      const atIndex = newInput.indexOf("@");
      const afterAt = newInput.substring(atIndex + 1);
  ...
  const commandSuggestions: CommandSuggestion[] = [
    ...
    { command: "/models", description: "Switch MiniMax Model" },
...
Enhanced Input Features:
  ...
  @filename   - Reference files with autocomplete (e.g., @package.json)
...
Model Configuration:
  Edit ~/.minimax/models.json to add custom models (Claude, GPT, Gemini, etc.)
